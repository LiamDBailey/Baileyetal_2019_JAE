...
full_mod <- fitme(Vacated ~ log(Median3) * Year_group + Status + (1|Sub_Area), family = "binomial", data = VacatedTerritories, method = "PQL/L")
...
vif(glm(Vacated ~ log(Median3) + Year_group + Status, family = "binomial", data = VacatedTerritories))
...
CI_mod <- glmer(Vacated ~ log(Median3) * Year_group + Status + (1|Sub_Area),
...
CIs <- as.data.frame(cbind(confint(CI_mod, method = "Wald")[-1, ],
  ...
  mutate(CI95 = paste(round(`2.5 %`, 4), round(`97.5 %`, 4), sep = "/"),
         ...
         Estimate = format(as.numeric(fixef(CI_mod))),
