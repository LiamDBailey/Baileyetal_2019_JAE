...
Nest_elevation <- Nest_elevation %>%
  mutate(#Make elevation cm above 1971 MHT
         ...
         Area = factor(pmap_chr(.l = list(Area = Area),
                                .f = function(Area){
                                 
                                  return(ifelse(grepl("B", Area), "B", ifelse(grepl("F", Area), "F", as.character(Area)))) 
                                  
                                })),
         AreaB = factor(pmap_chr(.l = list(Area = as.character(Area)),
...
mod1 <- lmer(Z_min ~ Year1 + Method + (1|Area) + (1|MaleID),
...
mod2 <- lmer(Z_bc ~ Year1 + Method + (1|Area) + (1|MaleID),
            data = Nest_elevation, REML = TRUE)
...
elev_change <- pmap_dbl(.l = list(Year1 = 1:max(Nest_elevation$Year1)),
                        .f = function(Year1){
...
intervals <- pmap(.l = list(parm = as.character(CIs$Variable)),
     .f = function(parm){
...
CIs <- CIs %>% 
  mutate(CI95 = pmap_chr(.l = list(intervals),
                          .f = ~paste(format(..1[1], nsmall = 2), format(..1[2], nsmall = 2), sep = "/")),
         CI995 = pmap_chr(.l = list(intervals),
