...
library(spaMM)
library(lme4)
full_mod <- fitme(lgl_Settle ~ Year_group * log(Median3) + Grid_area2 + (1|Sub_Area),
...
*N.B.* glmer throws a convergence warning, but model is able to produce identical estimates to spaMM. lme4 is known to be overly conservative with throwing convergence warnings.

```{r}

library(kableExtra)

CI_mod <- glmer(lgl_Settle ~ Year_group * log(Median3) + Grid_area2 + (1|Sub_Area),
...
CIs <- as.data.frame(cbind(confint(CI_mod, method = "Wald")[-1, ],
  ...
  mutate(CI95 = paste(round(`2.5 %`, 4), round(`97.5 %`, 4), sep = "/"),
         ...
         Estimate = as.numeric(fixef(CI_mod)),
...
library(spaMM)
full_mod_100 <- fitme(lgl_Settle ~ Year_group * log(Median3) + Grid_area2 + (1|Sub_Area),
