...
devtools::load_all()
devtools::install_github("LiamDBailey/MyFuncs", upgrade = "nevers")
...
library(spaMM)
full_mod <- fitme(lgl_Settle ~ Year_group * log(Median3) + log(Coast_Dist2) + log(Gully_Dist2) + rel_fledge_est3yr + rel_density3yr + Grid_area2 + (1|Sub_Area),
...
SETTLE_poisson %>%
  ...
  select(logits, Year, Median2, rel_fledge_est3yr, rel_density3yr) %>%
...
simResiduals <- simulateResiduals(fittedModel = full_mod, n = 5000)

plotSimulatedResiduals(simulationOutput = simResiduals)
...
vif(glm(lgl_Settle ~ Year_group + log(Median3) + log(Coast_Dist2) + log(Gully_Dist2) + Grid_area2 + rel_fledge_est3yr + rel_density3yr, family = "binomial", data = SETTLE_poisson))
...
qqnorm(ranef(full_mod)[[1]])
qqline(ranef(full_mod)[[1]])
...
all_models <- expand.grid(elev = c("", "+ Year_group : log(Median3)"),
  ...
  rowwise() %>% 
  mutate(models_spamm = list(fitme(as.formula(mod_formula), data = SETTLE_poisson, family = binomial(logit), method = "PQL/L")),
         models_lme4 = list(glmer(as.formula(mod_formula), data = SETTLE_poisson, family = binomial(logit))))

#This null model with lme4 is needed to improve fitting for R2
#Otherwise, the null model is re-estimated every time.
null_model <- glmer(lgl_Settle ~ 1 + (1|Sub_Area),
...
(selection_table <- all_models %>% 
  ...
  select(elev:fledge, cAIC, deltaAIC:Conf_set, R2m, R2c, models_spamm, models_lme4, -mod_formula, -relative_loglik, -R2s))
...
Print selection table with kable
...
bin_dat <- SETTLE_poisson %>%
  ...
  mutate(Density_numeric = pmap_dbl(.l = list(Density_class = as.character(.$Density_class)),
...
CIs <- as.data.frame(cbind(confint(selection_table$models_lme4[[1]], method = "Wald")[-1, ],
  ...
  ungroup() %>%
  mutate(Estimate = format(round((fixef(selection_table$models_lme4[[1]])), digits = 2), nsmall = 2),
         Variable = names(fixef(selection_table$models_lme4[[1]]))) %>% 
  select(Variable, Estimate, CI95, CI995)
...
library(spaMM)
full_mod_100 <- fitme(lgl_Settle ~ Year_group * log(Median3) + rel_fledge_est3yr + rel_density3yr + Grid_area2 + (1|Sub_Area),
...
simResiduals <- simulateResiduals(fittedModel = full_mod_100, n = 5000)
...
vif(glm(lgl_Settle ~ Year_group + log(Median3) + log(Coast_Dist2) + log(Gully_Dist2) + rel_fledge_est3yr + Grid_area2 + rel_density3yr, family = "binomial", data = SETTLE_poisson_100))
...
qqnorm(ranef(full_mod_100)[[1]])
qqline(ranef(full_mod_100)[[1]])
...
CI_mod <- glmer(lgl_Settle ~ Year_group * log(Median3) + Grid_area2 + rel_density3yr + rel_fledge_est3yr + (1|Sub_Area),
...
CIs <- as.data.frame(cbind(confint(CI_mod, method = "Wald")[-1, ],
  ...
  mutate(CI95 = paste(format(`2.5 %`, digits = 2, scientific = -2, nsmall = 2), format(`97.5 %`, digits = 2, scientific = -2, nsmall = 2), sep = "/"),
         CI995 = paste(format(`0.25 %`, digits = 2, scientific = -2, nsmall = 2), format(`99.75 %`, digits = 2, scientific = -2, nsmall = 2), sep = "/")) %>% 
  ungroup() %>%
  mutate(Estimate = format(round((fixef(CI_mod)), digits = 2), nsmall = 2),
         Variable = names(fixef(CI_mod))) %>% 
  select(Variable, Estimate, CI95, CI995)
